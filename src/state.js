const store = {
	// "main": {
		// name: 'Main'
	// },
	"set1": {
		name: 'Action (set A)',
		imgSrc: './assets/img/jump.jpg',
		children: [
			{
			  word: 'cry',
			  translation: 'плакать',
			  imgSrc: 'assets/img/cry.jpg',
			  audioSrc: 'assets/audio/cry.mp3'
			},
			{
			  word: 'dance',
			  translation: 'танцевать',
			  imgSrc: 'assets/img/dance.jpg',
			  audioSrc: 'assets/audio/dance.mp3'
			},
			{
			  word: 'dive',
			  translation: 'нырять',
			  imgSrc: 'assets/img/dive.jpg',
			  audioSrc: 'assets/audio/dive.mp3'
			},
			{
			  word: 'draw',
			  translation: 'рисовать',
			  imgSrc: 'assets/img/draw.jpg',
			  audioSrc: 'assets/audio/draw.mp3'
			},
			{
			  word: 'fish',
			  translation: 'ловить рыбу',
			  imgSrc: 'assets/img/fish.jpg',
			  audioSrc: 'assets/audio/fish.mp3'
			},
			{
			  word: 'fly',
			  translation: 'летать',
			  imgSrc: 'assets/img/fly.jpg',
			  audioSrc: 'assets/audio/fly.mp3'
			},
			{
			  word: 'hug',
			  translation: 'обнимать',
			  imgSrc: 'assets/img/hug.jpg',
			  audioSrc: 'assets/audio/hug.mp3'
			},
			{
			  word: 'jump',
			  translation: 'прыгать',
			  imgSrc: 'assets/img/jump.jpg',
			  audioSrc: 'assets/audio/jump.mp3'
			}
		 ],
	},
	"set2": {
		name: 'Action (set B)',
		imgSrc: './assets/img/open.jpg',
		children: [
			{
			  word: 'open',
			  translation: 'открывать',
			  imgSrc: 'assets/img/open.jpg',
			  audioSrc: 'assets/audio/open.mp3'
			},
			{
			  word: 'play',
			  translation: 'играть',
			  imgSrc: 'assets/img/play.jpg',
			  audioSrc: 'assets/audio/play.mp3'
			},
			{
			  word: 'point',
			  translation: 'указывать',
			  imgSrc: 'assets/img/point.jpg',
			  audioSrc: 'assets/audio/point.mp3'
			},
			{
			  word: 'ride',
			  translation: 'ездить',
			  imgSrc: 'assets/img/ride.jpg',
			  audioSrc: 'assets/audio/ride.mp3'
			},
			{
			  word: 'run',
			  translation: 'бегать',
			  imgSrc: 'assets/img/run.jpg',
			  audioSrc: 'assets/audio/run.mp3'
			},
			{
			  word: 'sing',
			  translation: 'петь',
			  imgSrc: 'assets/img/sing.jpg',
			  audioSrc: 'assets/audio/sing.mp3'
			},
			{
			  word: 'skip',
			  translation: 'пропускать, прыгать',
			  imgSrc: 'assets/img/skip.jpg',
			  audioSrc: 'assets/audio/skip.mp3'
			},
			{
			  word: 'swim',
			  translation: 'плавать',
			  imgSrc: 'assets/img/swim.jpg',
			  audioSrc: 'assets/audio/swim.mp3'
			}
		 ]
	},
	"animal1": {
		name: 'Animal (set A)',
		imgSrc: './assets/img/cat.jpg',
		children: [
			{
			  word: 'cat',
			  translation: 'кот',
			  imgSrc: 'assets/img/cat.jpg',
			  audioSrc: 'assets/audio/cat.mp3'
			},
			{
			  word: 'chick',
			  translation: 'цыплёнок',
			  imgSrc: 'assets/img/chick.jpg',
			  audioSrc: 'assets/audio/chick.mp3'
			},
			// {
			//   word: 'chicken',
			//   translation: 'курица',
			//   imgSrc: 'assets/img/chicken.jpg',
			//   audioSrc: 'assets/audio/chicken.mp3'
			// },
			// {
			//   word: 'dog',
			//   translation: 'собака',
			//   imgSrc: 'assets/img/dog.jpg',
			//   audioSrc: 'assets/audio/dog.mp3'
			// },
			// {
			//   word: 'horse',
			//   translation: 'лошадь',
			//   imgSrc: 'assets/img/horse.jpg',
			//   audioSrc: 'assets/audio/horse.mp3'
			// },
			// {
			//   word: 'pig',
			//   translation: 'свинья',
			//   imgSrc: 'assets/img/pig.jpg',
			//   audioSrc: 'assets/audio/pig.mp3'
			// },
			// {
			//   word: 'rabbit',
			//   translation: 'кролик',
			//   imgSrc: 'assets/img/rabbit.jpg',
			//   audioSrc: 'assets/audio/rabbit.mp3'
			// },
			{
			  word: 'sheep',
			  translation: 'овца',
			  imgSrc: 'assets/img/sheep.jpg',
			  audioSrc: 'assets/audio/sheep.mp3'
			}
		 ]

	},
	"animal2": {
		name: 'Animal (set B)',
		imgSrc: './assets/img/bird.jpg',
		children: [
			{
			  word: 'bird',
			  translation: 'птица',
			  imgSrc: 'assets/img/bird.jpg',
			  audioSrc: 'assets/audio/bird.mp3'
			},
			{
			  word: 'fish',
			  translation: 'рыба',
			  imgSrc: 'assets/img/fish1.jpg',
			  audioSrc: 'assets/audio/fish.mp3'
			},
			{
			  word: 'frog',
			  translation: 'жаба',
			  imgSrc: 'assets/img/frog.jpg',
			  audioSrc: 'assets/audio/frog.mp3'
			},
			{
			  word: 'giraffe',
			  translation: 'жирафа',
			  imgSrc: 'assets/img/giraffe.jpg',
			  audioSrc: 'assets/audio/giraffe.mp3'
			},
			{
			  word: 'lion',
			  translation: 'лев',
			  imgSrc: 'assets/img/lion.jpg',
			  audioSrc: 'assets/audio/lion.mp3'
			},
			{
			  word: 'mouse',
			  translation: 'мышь',
			  imgSrc: 'assets/img/mouse.jpg',
			  audioSrc: 'assets/audio/mouse.mp3'
			},
			{
			  word: 'turtle',
			  translation: 'черепаха',
			  imgSrc: 'assets/img/turtle.jpg',
			  audioSrc: 'assets/audio/turtle.mp3'
			},
			{
			  word: 'dolphin',
			  translation: 'дельфин',
			  imgSrc: 'assets/img/dolphin.jpg',
			  audioSrc: 'assets/audio/dolphin.mp3'
			}
		 ]
	},
	"clothes": {
		name: 'Clothes',
		imgSrc: './assets/img/dress.jpg',
		children: [
			{
			  word: 'skirt',
			  translation: 'юбка',
			  imgSrc: 'assets/img/skirt.jpg',
			  audioSrc: 'assets/audio/skirt.mp3'
			},
			{
			  word: 'pants',
			  translation: 'брюки',
			  imgSrc: 'assets/img/pants.jpg',
			  audioSrc: 'assets/audio/pants.mp3'
			},
			{
			  word: 'blouse',
			  translation: 'блузка',
			  imgSrc: 'assets/img/blouse.jpg',
			  audioSrc: 'assets/audio/blouse.mp3'
			},
			{
			  word: 'dress',
			  translation: 'платье',
			  imgSrc: 'assets/img/dress.jpg',
			  audioSrc: 'assets/audio/dress.mp3'
			},
			{
			  word: 'boot',
			  translation: 'ботинок',
			  imgSrc: 'assets/img/boot.jpg',
			  audioSrc: 'assets/audio/boot.mp3'
			},
			{
			  word: 'shirt',
			  translation: 'рубашка',
			  imgSrc: 'assets/img/shirt.jpg',
			  audioSrc: 'assets/audio/shirt.mp3'
			},
			{
			  word: 'coat',
			  translation: 'пальто',
			  imgSrc: 'assets/img/coat.jpg',
			  audioSrc: 'assets/audio/coat.mp3'
			},
			{
			  word: 'shoe',
			  translation: 'туфли',
			  imgSrc: 'assets/img/shoe.jpg',
			  audioSrc: 'assets/audio/shoe.mp3'
			}
		]
	},
	"emotion": {
		name: 'Emotions',
		imgSrc: './assets/img/happy.jpg',
		children: [
			{
			  word: 'sad',
			  translation: 'грустный',
			  imgSrc: 'assets/img/sad.jpg',
			  audioSrc: 'assets/audio/sad.mp3'
			},
			{
			  word: 'angry',
			  translation: 'сердитый',
			  imgSrc: 'assets/img/angry.jpg',
			  audioSrc: 'assets/audio/angry.mp3'
			},
			{
			  word: 'happy',
			  translation: 'счастливый',
			  imgSrc: 'assets/img/happy.jpg',
			  audioSrc: 'assets/audio/happy.mp3'
			},
			{
			  word: 'tired',
			  translation: 'уставший',
			  imgSrc: 'assets/img/tired.jpg',
			  audioSrc: 'assets/audio/tired.mp3'
			},
			{
			  word: 'surprised',
			  translation: 'удивлённый',
			  imgSrc: 'assets/img/surprised.jpg',
			  audioSrc: 'assets/audio/surprised.mp3'
			},
			{
			  word: 'scared',
			  translation: 'испуганный',
			  imgSrc: 'assets/img/scared.jpg',
			  audioSrc: 'assets/audio/scared.mp3'
			},
			{
			  word: 'smile',
			  translation: 'улыбка',
			  imgSrc: 'assets/img/smile.jpg',
			  audioSrc: 'assets/audio/smile.mp3'
			},
			{
			  word: 'laugh',
			  translation: 'смех',
			  imgSrc: 'assets/img/laugh.jpg',
			  audioSrc: 'assets/audio/laugh.mp3'
			}
		]
	},
	"food": {
		name: 'Food',
		imgSrc: './assets/img/potato.jpg',
		children: [
			{
			  word: 'pasta',
			  translation: 'макароны',
			  imgSrc: 'assets/img/pasta.jpg',
			  audioSrc: 'assets/audio/pasta.mp3'
			},
			{
			  word: 'potato',
			  translation: 'картошка',
			  imgSrc: 'assets/img/potato.jpg',
			  audioSrc: 'assets/audio/potato.mp3'
			},
			{
			  word: 'cutlet',
			  translation: 'котлета',
			  imgSrc: 'assets/img/cutlet.jpg',
			  audioSrc: 'assets/audio/cutlet.mp3'
			},
			{
			  word: 'salad',
			  translation: 'салат',
			  imgSrc: 'assets/img/salad.jpg',
			  audioSrc: 'assets/audio/salad.mp3'
			},
			{
			  word: 'cucumber',
			  translation: 'огурец',
			  imgSrc: 'assets/img/cucumber.jpg',
			  audioSrc: 'assets/audio/cucumber.mp3'
			},
			{
			  word: 'sausages',
			  translation: 'сосиски',
			  imgSrc: 'assets/img/sausages.jpg',
			  audioSrc: 'assets/audio/sausages.mp3'
			},
			{
			  word: 'meat',
			  translation: 'мясо',
			  imgSrc: 'assets/img/meat.jpg',
			  audioSrc: 'assets/audio/meat.mp3'
			},
			{
			  word: 'porridge',
			  translation: 'каша',
			  imgSrc: 'assets/img/porridge.jpg',
			  audioSrc: 'assets/audio/porridge.mp3'
			}
		]
	},
	"fruit": {
		name: 'Fruit',
		imgSrc: './assets/img/watermelon.jpg',
		children: [
			{
			  word: 'apple',
			  translation: 'яблоко',
			  imgSrc: 'assets/img/apple.jpg',
			  audioSrc: 'assets/audio/apple.mp3'
			},
			{
			  word: 'lemon',
			  translation: 'лемон',
			  imgSrc: 'assets/img/lemon.jpg',
			  audioSrc: 'assets/audio/lemon.mp3'
			},
			{
			  word: 'banana',
			  translation: 'банан',
			  imgSrc: 'assets/img/banana.jpg',
			  audioSrc: 'assets/audio/banana.mp3'
			},
			{
			  word: 'watermelon',
			  translation: 'арбуз',
			  imgSrc: 'assets/img/watermelon.jpg',
			  audioSrc: 'assets/audio/watermelon.mp3'
			},
			{
			  word: 'raspberry',
			  translation: 'малина',
			  imgSrc: 'assets/img/raspberry.jpg',
			  audioSrc: 'assets/audio/raspberry.mp3'
			},
			{
			  word: 'orange',
			  translation: 'апельсин',
			  imgSrc: 'assets/img/orange.jpg',
			  audioSrc: 'assets/audio/orange.mp3'
			},
			{
			  word: 'strawberry',
			  translation: 'клубника',
			  imgSrc: 'assets/img/strawberry.jpg',
			  audioSrc: 'assets/audio/strawberry.mp3'
			},
			{
			  word: 'pear',
			  translation: 'груша',
			  imgSrc: 'assets/img/pear.jpg',
			  audioSrc: 'assets/audio/pear.mp3'
			}
		]
	}
}

export default store;
window.store = store;